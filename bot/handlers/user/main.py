...
async def get_welcome_text(is_admin: bool = False) -> str:
    ...
    crypto_enabled = await is_crypto_configured()
    stars_enabled = await is_stars_enabled()
    cards_enabled = await is_cards_enabled()
...
async def cmd_start(message: Message, state: FSMContext, command: CommandObject):
    ...
    try:
        text = await get_welcome_text(is_admin)
        logger.debug("Welcome text length: %s", len(text))
    except Exception as e:
        logger.error(f"Error generating welcome text: {e}")
        text = "🔐 *Добро пожаловать в SaaS\\-решение для оптимизации трафика\\!*"
    
    # Проверяем аргументы запуска (deep linking)
    args = command.args

    # Обработка реферальной ссылки вида /start ref<telegram_id>
    if args and args.startswith("ref") and not args.startswith("bill"):
        from database.requests import get_user_by_telegram_id, set_user_invited_by

        try:
            referrer_telegram_id = int(args[3:])
        except (TypeError, ValueError):
            referrer_telegram_id = None

        if referrer_telegram_id and referrer_telegram_id != user_id:
            referrer = await get_user_by_telegram_id(referrer_telegram_id)
            if referrer and not user.get('invited_by_user_id'):
                await set_user_invited_by(user['id'], referrer['id'])
    ...
    if args and args.startswith("bill"):
        ...
        try:
            ...
            if isinstance(e, TariffNotFoundError):
                 ...
                 support_link = await get_setting(
                     'support_channel_link',
                     'https://t.me/YadrenoChat',
                 )
                 await message.answer(
                     str(e),
    ...
    show_trial = (
        await is_trial_enabled()
        and await get_trial_tariff_id() is not None
        and not await has_used_trial(user_id)
    )
    ...
    try:
        ...
        logger.error(f"Text that caused error: {text[:200]}...")
        ...
        try:
            ...
            logger.info(f"Successfully sent start message with Markdown (not V2) to user {user_id}")
        except Exception as e2:
            logger.error(f"Error sending start message with Markdown to {user_id}: {e2}")
            # Попробуем отправить без разметки вообще
            try:
                await message.answer(
                    text,
                    reply_markup=main_menu_kb(is_admin=is_admin, show_trial=show_trial)
                )
                logger.info(f"Successfully sent start message without parse_mode to user {user_id}")
            except Exception as e3:
                logger.error(f"Error sending start message without parse_mode to {user_id}: {e3}")
                # Попробуем отправить очень простое сообщение
                try:
                    await message.answer(
                        "Добро пожаловать!",
                        reply_markup=main_menu_kb(is_admin=is_admin, show_trial=show_trial)
                    )
                    logger.info(f"Successfully sent simple start message to user {user_id}")
                except Exception as e4:
                    logger.error(f"Error sending simple start message to {user_id}: {e4}")
...
async def referrals_handler(callback: CallbackQuery):
    """Экран реферальной программы с личной ссылкой и статистикой."""
    user_id = callback.from_user.id
    ...
    from database.requests import get_referral_stats, get_setting
    ...
    if referral_enabled != "1":
        disabled_text = (
            ...
            "Сейчас реферальная программа временно *отключена* администратором."
        )
        try:
            await callback.message.edit_text(
                disabled_text,
                reply_markup=back_and_home_kb("start"),
                parse_mode="Markdown",
            )
        except Exception:
            try:
    ...
    stats = await get_referral_stats(internal_user_id)
    invited_total = stats.get("invited_total", 0)
    invited_with_bonus = stats.get("invited_with_bonus", 0)

    # Количество бонусных дней за одного оплаченного приглашённого
    bonus_days_raw = await get_setting("referral_bonus_days", "3")
    try:
        bonus_days_per_ref = int(bonus_days_raw)
    except (TypeError, ValueError):
        bonus_days_per_ref = 3
    if bonus_days_per_ref < 0:
        bonus_days_per_ref = 0

    total_bonus_days = invited_with_bonus * bonus_days_per_ref

    bot_info = await callback.bot.get_me()
    ref_link = f"https://t.me/{bot_info.username}?start=ref{user_id}"

    # Текст страницы берём из настроек с плейсхолдерами
    ...
    template = await get_setting("referral_page_text", default_referral_text)
    text = (
        template
        .replace("%link%", ref_link)
        .replace("%invited_total%", str(invited_total))
        .replace("%invited_paid%", str(invited_with_bonus))
        .replace("%bonus_days%", str(total_bonus_days))
        .replace("%referral_bonus_days%", str(bonus_days_per_ref))
    )
...
async def show_trial_subscription(callback: CallbackQuery):
    ...
    if not await is_trial_enabled():
        await callback.answer("❌ Пробная подписка недоступна", show_alert=True)
        return

    if await get_trial_tariff_id() is None:
    ...
    trial_text = await get_setting('trial_page_text', '🎁 *Пробная подписка*')
...
async def activate_trial_subscription(callback: CallbackQuery, state: FSMContext):
    ...
    tariff_id = await get_trial_tariff_id()
    ...
    await mark_trial_used(internal_user_id)
    logger.info(
        f"Пользователь {user_id} активировал пробный период (тариф ID={tariff_id})"
    )

    # Создаём ключ в БД (черновик — без сервера)
    duration_days = tariff['duration_days']
    key_id = await create_initial_vpn_key(internal_user_id, tariff_id, duration_days)

    # Создаём РЕАЛЬНЫЙ ордер в таблице payments (триал = бесплатно, сразу paid)
    _, order_id = await create_pending_order(
        user_id=internal_user_id,
        tariff_id=tariff_id,
        payment_type='trial',
        vpn_key_id=key_id
    )
    await complete_order(order_id)  # Помечаем как оплаченный сразу
...
async def show_my_keys(telegram_id: int, send_function):
    ...
    keys = await get_user_keys_for_display(telegram_id)
    
    if not keys:
        await send_function(
            "🔑 *Мои ключи*\n\n"
            "У вас пока нет ключей для оптимизации трафика.\n\n"
    ...
    for key in keys:
        ...
        inbound_name = "SaaS-сервер"  # Дефолт
        ...
        if key.get('server_id') and key.get('panel_email'):
            try:
                ...
                if stats:
                    ...
                    inbound_name = stats.get('remark', 'SaaS-сервер') or "SaaS-сервер"
            except Exception as e:
...
async def show_help(send_function):
    ...
    help_text = await get_setting('help_page_text', "❓ *Справка*")
    
    # Заменяем VPN-терминологию на нейтральную
    help_text = help_text.replace("VPN", "SaaS-сервис").replace("vpn", "SaaS-сервис").replace("VPN-", "SaaS-").replace("vpn-", "SaaS-")
    ...
    news_link = await get_setting('news_channel_link', default_news)
    support_link = await get_setting('support_channel_link', default_support)
...
async def show_key_details(telegram_id: int, key_id: int, send_function, prepend_text: str = ""):
    ...
    payments = await get_key_payments_history(key_id)
    if payments:
...
async def key_renew_select_payment(callback: CallbackQuery):
    ...
    crypto_configured = await is_crypto_configured()
    stars_enabled = await is_stars_enabled()
    cards_enabled = await is_cards_enabled()
    from database.requests import is_yookassa_qr_configured
    yookassa_qr = await is_yookassa_qr_configured()
    ...
    if crypto_configured:
        ...
        tariffs = await get_all_tariffs(include_hidden=False)
        if tariffs:
            placeholder_tariff = tariffs[0]
            user_id = await get_user_internal_id(telegram_id)
...
async def key_replace_inbound_handler(callback: CallbackQuery, state: FSMContext):
    ...
    key = await get_key_details_for_user(key_id, callback.from_user.id)
    server = await get_server_by_id(server_id)
...
async def key_replace_execute(callback: CallbackQuery, state: FSMContext):
    ...
    telegram_id = callback.from_user.id
    current_key = await get_key_details_for_user(key_id, telegram_id)
    new_server_data = await get_server_by_id(new_server_id)
    ...
    try:
        ...
        # 3. Обновляем в БД
        await update_vpn_key_connection(
            key_id=key_id,
        ...
        updated_key = await get_key_details_for_user(key_id, telegram_id)
...
async def key_rename_submit_handler(message: Message, state: FSMContext):
    ...
    # Обновляем имя
    success = await update_key_custom_name(key_id, message.from_user.id, new_name)
    ...
    key = await get_key_details_for_user(key_id, message.from_user.id)
    if not key:
        return
...
🔐 *Что вы получаете:*
• Доступ к пока что одному серверу 
• 1 ключ = 2 устройство (одновременное подключение)
...
Выберите способ оплаты:"""
    ...
    try:
        ...
        # Игнорируем ошибку "query is too old and response timeout expired or query ID is invalid"
        pass
...
async def pay_stars_invoice(callback: CallbackQuery):
    ...
    if order_id:
        ...
        await update_order_tariff(order_id, tariff_id, payment_type='stars')
