...
async def handle_start_with_payment(message: Message, command: CommandObject, state: FSMContext):
    ...
    # Обрабатываем платёж
    try:
        success, response_text, order = await process_crypto_payment(start_param, user_id=user_id)
        ...
        if isinstance(e, TariffNotFoundError):
            ...
            support_link = await get_setting('support_channel_link', 'https://t.me/YadrenoChat')
            await message.answer(
                str(e),
...
# ============================================================================
# РУЧНАЯ ОПЛАТА ПЕРЕВОДОМ НА КАРТУ
# ============================================================================

@router.callback_query(F.data == "pay_manual_card")
async def pay_manual_card_select_tariff(callback: CallbackQuery):
    """
    Выбор тарифа для ручной оплаты переводом на карту (новый ключ).
    Пользователь затем отправляет скриншот чека, который уходит администратору.
    """
    from database.requests import get_all_tariffs
    from bot.keyboards.user import manual_card_tariff_select_kb
    from bot.keyboards.admin import home_only_kb
    ...
    if not rub_tariffs:
        await callback.message.edit_text(
            ...
            "😔 Для этого способа не настроены тарифы с ценой в рублях.\n"
    ...
    await callback.message.edit_text(
        "💸 *Оплата переводом на карту*\n\n"
        "Выберите тариф, после чего бот покажет реквизиты и попросит отправить скриншот чека.",
        reply_markup=manual_card_tariff_select_kb(rub_tariffs),
        parse_mode="Markdown"
    )
    await callback.answer()
...
async def manual_card_show_details(callback: CallbackQuery, state: FSMContext):
    """
    Показывает реквизиты для перевода на карту и запрашивает скриншот чека.
    """
    from database.requests import get_tariff_by_id, get_setting
    from bot.states.user_states import ManualCardPayment
    ...
    if price_rub <= 0:
        await callback.answer("❌ Для тарифа не задана цена в рублях", show_alert=True)
        return

    card_number = (await get_setting('manual_card_number', '') or '').strip()
    card_recipient = (await get_setting('manual_card_recipient', '') or '').strip()
    if card_number and card_recipient:
        card_text = f"{card_number}\nПолучатель: {card_recipient}"
    else:
        card_text = "2204321030120737"
    ...
    text = (
        ...
        f"`{card_text}`\n\n"
        "После успешной оплаты *отправьте сюда скриншот или фото чека* одним сообщением.\n\n"
        "_После проверки администратор подтвердит оплату и выдаст вам ключ._"
    )

    await state.set_state(ManualCardPayment.waiting_for_screenshot)
...
@router.message(StateFilter("ManualCardPayment:waiting_for_screenshot"))
async def manual_card_receive_screenshot(message: Message, state: FSMContext):
    """
    Принимает скриншот чека при ручной оплате и пересылает его администраторам.
    Хендлер срабатывает только в состоянии ManualCardPayment.waiting_for_screenshot.
    """
    from bot.states.user_states import ManualCardPayment
    from bot.keyboards.admin import manual_payment_review_kb

    if not (message.photo or message.document):
        await message.answer(
            "📸 Пожалуйста, отправьте *скриншот или фото чека* одним сообщением.\n"
            "Если вы передумали — вернитесь в главное меню через /start.",
            parse_mode="Markdown"
        )
        return

    data = await state.get_data()
    info = data.get("manual_payment", {}) or {}
    tariff_id = info.get("tariff_id")
    tariff_name = info.get("tariff_name", "Тариф")
    price_rub = info.get("price_rub", 0)

    user = message.from_user
    username = f"@{user.username}" if user.username else "нет username"
    caption = (
        "🧾 *Новый чек ручной оплаты на карту*\n\n"
        f"👤 Пользователь: {username}\n"
        f"🆔 Telegram ID: `{user.id}`\n"
        f"💳 Тариф: {tariff_name}\n"
        f"💰 Сумма: {price_rub} ₽\n\n"
        "Проверьте поступление на карту и подтвердите или отклоните оплату.\n"
        "_При подтверждении ключ будет выдан автоматически._"
    )

    # Пересылаем чек всем администраторам (с tariff_id для авто-выдачи ключа)
    for admin_id in ADMIN_IDS:
        try:
            await message.copy_to(
                admin_id,
                caption=caption,
                reply_markup=manual_payment_review_kb(user_telegram_id=user.id, tariff_id=tariff_id or 0),
                parse_mode="Markdown"
            )
        except Exception as e:
            logger.warning(f"Не удалось отправить чек администратору {admin_id}: {e}")

    await state.clear()

    await message.answer(
        "✅ Чек отправлен администратору.\n"
        "Как только оплата будет подтверждена, админ выдаст вам ключ или свяжется с вами.",
        parse_mode="Markdown"
    )
...
async def renew_stars_invoice(callback: CallbackQuery):
    ...
    if order_id:
         ...
         await update_order_tariff(order_id, tariff_id)
         await update_payment_type(order_id, 'stars')
    else:
         # Создаем новый
...
async def finalize_payment_ui(message: Message, state: FSMContext, text: str, order: dict, user_id: int):
    ...
    is_draft = False
    if key_id:
        key = await get_key_details_for_user(key_id, user_id)
        if key:
...
async def process_new_key_final(callback: CallbackQuery, state: FSMContext, server_id: int, inbound_id: int):
    ...
    order = await find_order_by_order_id(order_id)
    if not order:
    ...
    if not key_id:
        if order['vpn_key_id']:
            ...
            key_id = await create_initial_vpn_key(order['user_id'], order['tariff_id'], days)
            await update_payment_key_id(order_id, key_id)
    ...
    try:
        ...
        new_key = await get_key_details_for_user(key_id, telegram_id)
...
async def pay_cards_invoice(callback: CallbackQuery):
    ...
    provider_token = await get_setting('cards_provider_token', '')
    ...
    if order_id:
        await update_order_tariff(order_id, tariff_id, payment_type='cards')
    else:
        user_id = await get_user_internal_id(callback.from_user.id)
        if not user_id:
...
async def qr_pay_create(callback: CallbackQuery):
    ...
    try:
        ...
        await save_yookassa_payment_id(order_id, result['yookassa_payment_id'])
...
async def check_yookassa_payment(callback: CallbackQuery, state: FSMContext):
    ...
    if await is_order_already_paid(order_id):
        order = await find_order_by_order_id(order_id)
        if order:
    ...
    if status == 'succeeded':
        await update_payment_type(order_id, 'yookassa_qr')
