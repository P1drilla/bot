...
async def toggle_tariff(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await callback.answer("⚠️ Некорректные данные", show_alert=True)
        return
    new_status = await toggle_tariff_active(tariff_id)
...
async def start_add_tariff(callback: CallbackQuery, state: FSMContext):
    ...
    include_crypto = await is_crypto_enabled()
...
async def edit_tariff_value(message: Message, state: FSMContext):
    ...
    # Сохраняем в БД
    success = await update_tariff_field(tariff_id, param['key'], value)
