...
from database.requests import (
    ...
    get_active_servers, get_all_tariffs, get_tariff_by_id, get_server_by_id,
...
async def show_users_menu(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(users_filter='all', users_page=0, grant_sub_mode=False)
...
async def show_users_list(callback: CallbackQuery, state: FSMContext):
    ...
    grant_sub_mode = data.get('grant_sub_mode', False)
    
    await _show_users_page(callback, state, page, current_filter, grant_sub_mode)
...
async def set_users_filter(callback: CallbackQuery, state: FSMContext):
    ...
    await _show_users_page(callback, state, 0, filter_type, grant_sub_mode)
...
async def _show_users_page(
    ...
    page: int, 
    filter_type: str,
    grant_sub_mode: bool = False
):
    """Отображает страницу списка пользователей (обычный просмотр или выбор для выдачи подписки)."""
    offset = page * USERS_PER_PAGE
    users, total = await get_all_users_paginated(offset, USERS_PER_PAGE, filter_type)
    ...
    if grant_sub_mode:
        ...
        title = f"👥 *Пользователи* — {filter_name}"
    ...
    list_mode = 'grant_sub' if grant_sub_mode else 'view'
    await callback.message.edit_text(
        text,
        reply_markup=users_list_kb(users, page, total_pages, filter_type, mode=list_mode),
        parse_mode="Markdown"
    )
    await callback.answer()
...
# ============================================================================
# ВЫДАТЬ ПОДПИСКУ (выбор пользователя → тариф → сервер → ключ)
# ============================================================================

@router.callback_query(F.data == "admin_grant_sub")
async def start_grant_sub(callback: CallbackQuery, state: FSMContext):
    """Начало выдачи подписки: показываем список пользователей."""
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ Доступ запрещён", show_alert=True)
        return
    
    await state.set_state(AdminStates.users_list)
    await state.update_data(grant_sub_mode=True, users_page=0, users_filter='all')
    
    await _show_users_page(callback, state, 0, 'all', grant_sub_mode=True)


@router.callback_query(F.data.startswith("admin_grant_sub_user:"))
async def grant_sub_select_user(callback: CallbackQuery, state: FSMContext):
    """Выбран пользователь для выдачи подписки → показываем тарифы."""
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ Доступ запрещён", show_alert=True)
        return
    ...
    active_tariffs = [t for t in tariffs if t.get('is_active')]
    
    if not active_tariffs:
        await callback.answer("❌ Нет активных тарифов", show_alert=True)
        return
    
    await state.set_state(AdminStates.add_key_tariff)
    ...
    await callback.message.edit_text(
        ...
        "Выберите тариф:",
        reply_markup=add_key_tariffs_kb(active_tariffs),
        parse_mode="Markdown"
    )
    await callback.answer()
...
async def _format_user_card(user: dict) -> tuple[str, any]:
    ...
    # Статистика оплат
    payment_stats = await get_user_payments_stats(user['id'])
    lines.append("")
    lines.append("💳 *Оплаты:*")
...
async def confirm_ban_toggle(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await callback.answer("⚠️ Некорректные данные", show_alert=True)
        return
    new_status = await toggle_user_ban(telegram_id)
...
async def show_key_view(callback: CallbackQuery, state: FSMContext):
    ...
    payments_history = await get_key_payments_history(key_id)
    if payments_history:
        text += "\n💳 *История платежей:*\n"
...
async def process_key_extend(message: Message, state: FSMContext):
    ...
    success = await extend_vpn_key(key_id, days)
...
async def select_add_key_server(callback: CallbackQuery, state: FSMContext):
    ...
    data = await state.get_data()
    grant_sub_mode = data.get('add_key_grant_sub_mode', False)
    
    if grant_sub_mode:
        # Режим «Выдать подписку»: тариф уже выбран → показываем inbound'ы
        tariff_id = data.get('add_key_tariff_id')
        ...
        try:
            ...
            if not inbounds:
                await callback.answer("❌ На сервере нет доступных inbound'ов", show_alert=True)
                return
            ...
            logger.error(f"Ошибка получения inbound'ов: {e}")
            await callback.answer(f"❌ Ошибка: {e}", show_alert=True)
        except Exception as e:
            logger.error(f"Неожиданная ошибка при получении inbound'ов: {e}")
            await callback.answer("❌ Ошибка при получении inbound'ов", show_alert=True)
        return
    
    # Обычный поток: сервер выбран → показываем тарифы
    tariffs = await get_all_tariffs()
    active_tariffs = [t for t in tariffs if t.get('is_active')]
    if not active_tariffs:
...
@router.callback_query(F.data.startswith("admin_add_key_tariff:"))
async def select_add_key_tariff(callback: CallbackQuery, state: FSMContext):
    """Выбор тарифа для нового ключа."""
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ Доступ запрещён", show_alert=True)
        return
    ...
    if grant_sub_mode:
        # Режим «Выдать подписку»: тариф выбран → показываем выбор сервера
        servers = await get_active_servers()
        if not servers:
            await callback.answer("❌ Нет активных серверов", show_alert=True)
            return
        await state.set_state(AdminStates.add_key_server)
        await state.update_data(add_key_tariff_id=tariff_id)
        await callback.message.edit_text(
            f"🎁 *Выдать подписку*\n\n"
            f"📋 Тариф: {tariff['name']} ({tariff['duration_days']} дн.)\n\n"
            "Выберите сервер:",
    ...
    # Обычный поток (Добавить ключ): сервер уже выбран → показываем inbound'ы
...
async def select_add_key_inbound(callback: CallbackQuery, state: FSMContext):
    """Выбор inbound'а для нового ключа."""
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ Доступ запрещён", show_alert=True)
        return
    ...
    # Получаем данные для создания ключа
    server = await get_server_by_id(server_id)
    tariff = await get_tariff_by_id(tariff_id)
    
    if not server or not tariff:
        await callback.answer("Ошибка данных", show_alert=True)
        return
    
    # Генерируем email для панели
    user = await get_user_by_telegram_id(data.get('add_key_user_telegram_id'))
    ...
    # Формируем имя ключа
    key_name = f"{user.get('username') or user['telegram_id']}_admin"
    
    await state.set_state(AdminStates.add_key_confirm)
    await state.update_data(
        add_key_inbound_id=inbound_id,
        add_key_email=email,
        add_key_uuid=client_uuid,
        add_key_name=key_name
    )
    
    # Формируем текст подтверждения
    text = (
        f"➕ *Подтверждение добавления ключа*\n\n"
        f"👤 Пользователь: {format_user_display(user)}\n"
        f"🖥️ Сервер: {server['name']}\n"
        f"📋 Тариф: {tariff['name']} ({tariff['duration_days']} дн.)\n"
        f"📧 Email: {email}\n"
        f"🔑 UUID: {client_uuid[:8]}...\n"
        f"🏷️ Имя: {key_name}\n"
...
async def confirm_add_key(callback: CallbackQuery, state: FSMContext):
    ...
    required_keys = [
        'add_key_user_id', 'add_key_server_id', 'add_key_tariff_id',
        'add_key_inbound_id', 'add_key_email', 'add_key_uuid', 'add_key_name'
    ]
    ...
    # Получаем данные
    user_id = data['add_key_user_id']
    server_id = data['add_key_server_id']
    tariff_id = data['add_key_tariff_id']
    inbound_id = data['add_key_inbound_id']
    email = data['add_key_email']
    client_uuid = data['add_key_uuid']
    key_name = data['add_key_name']
    
    # Получаем тариф для расчёта срока
    tariff = await get_tariff_by_id(tariff_id)
    if not tariff:
        await callback.answer("❌ Тариф не найден", show_alert=True)
        return
    ...
    try:
        ...
        # Добавляем клиента на панели
        await client.add_client(
            inbound_id=inbound_id,
            email=email,
            uuid=client_uuid,
            name=key_name,
            total_gb=0,  # Без лимита
            expiry_days=days
        )
        
        # Создаём запись в БД
        key_id = await create_vpn_key_admin(
            user_id=user_id,
            server_id=server_id,
            tariff_id=tariff_id,
        ...
        await callback.answer("✅ Ключ успешно добавлен!", show_alert=True)
        ...
        logger.error(f"Неожиданная ошибка при создании ключа: {e}")
...
async def add_key_back(callback: CallbackQuery, state: FSMContext):
    """Кнопка «Назад» в процессе добавления ключа."""
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ Доступ запрещён", show_alert=True)
        return
    ...
    data = await state.get_data()
    grant_sub = data.get('add_key_grant_sub_mode', False)

    # С экрана подтверждения → возврат к выбору inbound
    if current == AdminStates.add_key_confirm:
        server_id = data.get('add_key_server_id')
        tariff_id = data.get('add_key_tariff_id')
        server = await get_server_by_id(server_id) if server_id else None
        tariff = await get_tariff_by_id(tariff_id) if tariff_id else None
        if not server or not tariff:
            await callback.answer("❌ Ошибка данных", show_alert=True)
            return
        try:
            ...
            inbounds = await client.get_inbounds()
            if not inbounds:
                await callback.answer("❌ Нет inbound'ов", show_alert=True)
                return
            await state.set_state(AdminStates.add_key_inbound)
            title = "🎁 *Выдать подписку*" if grant_sub else "➕ *Добавление ключа*"
            await callback.message.edit_text(
                f"{title}\n\n"
                f"🖥️ Сервер: {server['name']}\n"
                f"📋 Тариф: {tariff['name']} ({tariff['duration_days']} дн.)\n\n"
        ...
        except (VPNAPIError, Exception) as e:
            logger.warning(f"Ошибка при возврате к inbound: {e}")
            await callback.answer("❌ Ошибка", show_alert=True)
        await callback.answer()
        return

    # С выбора inbound → возврат к тарифу (grant_sub) или к серверу (обычный поток)
    if current == AdminStates.add_key_inbound:
        if grant_sub:
            tariffs = await get_all_tariffs()
            active = [t for t in tariffs if t.get('is_active')]
            if not active:
            ...
            await callback.message.edit_text(
                ...
                f"👤 Пользователь: {format_user_display(user)}\n\n"
                "Выберите тариф:",
                reply_markup=add_key_tariffs_kb(active),
                parse_mode="Markdown"
            )
        else:
            servers = await get_active_servers()
    ...
    # С выбора тарифа → возврат к серверу (grant_sub) или к списку пользователей (grant_sub)
    if current == AdminStates.add_key_tariff:
        if grant_sub:
            await state.set_state(AdminStates.users_list)
            await state.update_data(grant_sub_mode=True, users_page=0, users_filter='all')
            await _show_users_page(callback, state, 0, 'all', grant_sub_mode=True)
    ...
    # С выбора сервера → возврат к карточке пользователя (обычный) или к тарифам (grant_sub)
    if current == AdminStates.add_key_server:
        if grant_sub:
            tariffs = await get_all_tariffs()
            active = [t for t in tariffs if t.get('is_active')]
            if not active:
            ...
            telegram_id = data.get('add_key_user_telegram_id') or data.get('current_user_telegram_id')
            await state.set_state(AdminStates.user_view)
            if telegram_id:
                await _show_user_view_edit(callback, state, telegram_id)
            else:
...
@router.callback_query(F.data.in_({"admin_add_key_cancel", "admin_user_add_key_cancel"}))
async def cancel_add_key(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.answer("❌ Добавление ключа отменено")
    
    if grant_sub_mode:
        # Возврат в список пользователей для выдачи подписки
        await state.set_state(AdminStates.users_list)
        await state.update_data(grant_sub_mode=True, users_page=0, users_filter='all')
        await _show_users_page(callback, state, 0, 'all', grant_sub_mode=True)
    else:
        await state.set_state(AdminStates.user_view)
        telegram_id = data.get('current_user_telegram_id')
        if telegram_id:
            await _show_user_view_edit(callback, state, telegram_id)
...
@router.callback_query(F.data == "admin_users_cancel")
async def cancel_users_search(callback: CallbackQuery, state: FSMContext):
...
async def cancel_key_action(callback: CallbackQuery, state: FSMContext):
    """Отмена действия с ключом."""
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ Доступ запрещён", show_alert=True)
        return
...
@router.callback_query(F.data == "admin_user_cancel")
