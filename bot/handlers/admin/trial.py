...
async def show_trial_menu(callback: CallbackQuery):
    ...
    enabled = await is_trial_enabled()
...
async def admin_trial_toggle(callback: CallbackQuery):
    ...
    current = await is_trial_enabled()
    new_value = '0' if current else '1'
    await set_setting('trial_enabled', new_value)
...
async def admin_trial_edit_text_save(message: Message, state: FSMContext):
    ...
    await set_setting('trial_page_text', new_text)
...
async def admin_trial_select_tariff(callback: CallbackQuery):
    ...
    tariffs = await get_all_tariffs(include_hidden=True)
    selected_id = await get_trial_tariff_id()
...
async def admin_trial_set_tariff(callback: CallbackQuery):
    ...
    await set_setting('trial_tariff_id', str(tariff_id))
    logger.info(
