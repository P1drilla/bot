...
async def get_broadcast_message() -> dict | None:
    """
    ...
    """
    msg_json = await get_setting('broadcast_message')
    if msg_json:
        try:
...
async def save_broadcast_message(text: str, photo_file_id: str | None = None) -> None:
    """Сохраняет сообщение для рассылки."""
    data = {'text': text, 'photo_file_id': photo_file_id}
    await set_setting('broadcast_message', json.dumps(data, ensure_ascii=False))


async def is_broadcast_in_progress() -> bool:
    """Проверяет, идёт ли рассылка сейчас."""
    return await get_setting('broadcast_in_progress', '0') == '1'


async def set_broadcast_in_progress(value: bool) -> None:
    """Устанавливает флаг рассылки."""
    await set_setting('broadcast_in_progress', '1' if value else '0')
...
async def show_broadcast_menu(callback: CallbackQuery, state: FSMContext):
    ...
    msg_data = await get_broadcast_message()
    has_message = msg_data is not None and msg_data.get('text')
    
    current_filter = await get_setting('broadcast_filter', 'all')
    in_progress = await is_broadcast_in_progress()
    user_count = await count_users_for_broadcast(current_filter)
...
async def broadcast_save_message(message: Message, state: FSMContext):
    ...
    await save_broadcast_message(text, photo_file_id)
...
async def broadcast_set_filter(callback: CallbackQuery):
    ...
    # Если фильтр не изменился, просто показываем уведомление
    if filter_key == current_filter:
        await callback.answer(f"Фильтр уже установлен: {BROADCAST_FILTERS[filter_key]}", show_alert=True)
        return
    
    await set_setting('broadcast_filter', filter_key)
    
    # Обновляем экран
    msg_data = await get_broadcast_message()
    has_message = msg_data is not None and msg_data.get('text')
    in_progress = await is_broadcast_in_progress()
    user_count = await count_users_for_broadcast(filter_key)
...
async def broadcast_start(callback: CallbackQuery):
    ...
    if await is_broadcast_in_progress():
...
async def broadcast_confirm(callback: CallbackQuery, bot: Bot):
    ...
    current_filter = await get_setting('broadcast_filter', 'all')
    user_ids = await get_users_for_broadcast(current_filter)
    ...
    # Устанавливаем флаг
    await set_broadcast_in_progress(True)
    ...
    # Сбрасываем флаг
    await set_broadcast_in_progress(False)
...
async def broadcast_notifications(callback: CallbackQuery, state: FSMContext):
    ...
    days = int(await get_setting('notification_days', '3') or '3')
...
async def broadcast_notify_days(callback: CallbackQuery, state: FSMContext):
    ...
    current_days = await get_setting('notification_days', '3') or '3'
...
async def broadcast_save_notify_days(message: Message, state: FSMContext):
    ...
    await set_setting('notification_days', str(days))
...
async def broadcast_notify_text(callback: CallbackQuery, state: FSMContext):
    ...
    current_text = await get_setting('notification_text', '') or ''
...
async def broadcast_save_notify_text(message: Message, state: FSMContext):
    ...
    await set_setting('notification_text', message.text)
