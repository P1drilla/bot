...
def buy_key_kb(
    ...
    # Ручная оплата переводом на карту — без эквайринга
    # Всегда показываем, админ при желании может в тексте указать, что способ временно недоступен.
    builder.row(
        InlineKeyboardButton(text="💸 Перевод на карту (ручная)", callback_data="pay_manual_card")
    )
...
def manual_card_tariff_select_kb(tariffs: list) -> InlineKeyboardMarkup:
    """
    Клавиатура выбора тарифа для ручной оплаты переводом на карту.
    ...
    for tariff in tariffs:
        ...
        builder.row(
            InlineKeyboardButton(
                text=f"💸 {tariff['name']} — {price_rub} ₽",
                callback_data=f"manual_pay:{tariff['id']}"
            )
        )
