...
def update_check_kb(has_updates: bool = True) -> InlineKeyboardMarkup:
    """Клавиатура проверки обновлений бота."""
    builder = InlineKeyboardBuilder()
    ...
    if has_updates:
        builder.row(
            InlineKeyboardButton(
                ...
                callback_data="admin_update_bot_check"
            )
        )
        builder.row(
            InlineKeyboardButton(
...
def servers_menu_kb() -> InlineKeyboardMarkup:
    """Клавиатура меню управления серверами."""
    builder = InlineKeyboardBuilder()
...
def server_edit_field_kb(server_id: int) -> InlineKeyboardMarkup:
    """
    Клавиатура выбора поля для редактирования сервера.
    ...
    for field_key, field_name in fields:
        builder.row(
            InlineKeyboardButton(
                text=field_name,
                callback_data=f"admin_edit_server_field:{server_id}:{field_key}"
            )
        )
    
    # Навигация
    builder.row(
        back_button(f"admin_server_view:{server_id}"),
        home_button()
    )
...
def server_edit_kb(server_id: int) -> InlineKeyboardMarkup:
    """
    ...
    # Кнопка "Готово"
    builder.row(
        InlineKeyboardButton(text="✅ Готово", callback_data=f"admin_server_edit_done:{server_id}")
    )
...
def server_delete_confirm_kb(server_id: int) -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(
            text="✅ Да, удалить",
            callback_data=f"admin_delete_server_confirm:{server_id}"
        )
    )
    builder.row(
        InlineKeyboardButton(
...
def server_active_confirm_kb(server_id: int, is_active: bool) -> InlineKeyboardMarkup:
    """
    Клавиатура подтверждения переключения активности сервера.
    ...
    builder.row(
        InlineKeyboardButton(
            ...
            callback_data=f"admin_toggle_server_active_confirm:{server_id}"
        )
    )
    builder.row(
        InlineKeyboardButton(
...
def system_menu_kb() -> InlineKeyboardMarkup:
    """Клавиатура меню системных функций."""
    builder = InlineKeyboardBuilder()
    
    # Редактирование текстов (главная, справка и т.д.)
    builder.row(
        InlineKeyboardButton(text="📝 Тексты", callback_data="admin_edit_texts")
    )
    # Системные функции
    builder.row(
    ...
    builder.row(
        InlineKeyboardButton(text="⚙️ Перезагрузка", callback_data="admin_system_restart")
    )
    builder.row(
        InlineKeyboardButton(text="🗑️ Очистка", callback_data="admin_system_cleanup")
    )
...
def settings_list_kb() -> InlineKeyboardMarkup:
    """Клавиатура списка настроек."""
    builder = InlineKeyboardBuilder()
...
def setting_edit_kb(setting_key: str) -> InlineKeyboardMarkup:
    """
    ...
    # Кнопка "Готово"
    builder.row(
        InlineKeyboardButton(text="✅ Готово", callback_data=f"admin_setting_edit_done:{setting_key}")
    )
    
    # Навигация
    builder.row(
        back_button("admin_bot_settings"),
        home_button()
    )
...
def edit_texts_menu_kb() -> InlineKeyboardMarkup:
    """Клавиатура меню «Редактирование текстов» — только текстовые настройки."""
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text="📄 Главная страница", callback_data="admin_setting_edit:main_page_text")
    )
    builder.row(
        InlineKeyboardButton(text="❓ Справка", callback_data="admin_setting_edit:help_page_text")
    )
    builder.row(
        InlineKeyboardButton(text="🎁 Текст пробной подписки", callback_data="admin_setting_edit:trial_page_text")
    )
    builder.row(
        InlineKeyboardButton(text="⏰ Текст уведомления об истечении", callback_data="admin_setting_edit:notification_text")
    )
    builder.row(back_button("admin_bot_settings"), home_button())
    return builder.as_markup()


def setting_toggle_kb(setting_key: str, is_enabled: bool) -> InlineKeyboardMarkup:
    """
    Клавиатура переключения настройки.
    
    Args:
        setting_key: Ключ настройки
        is_enabled: Текущий статус включения
    """
    builder = InlineKeyboardBuilder()
    ...
    builder.row(
        InlineKeyboardButton(
            text=toggle_text,
            callback_data=f"admin_setting_toggle:{setting_key}"
        )
    )
...
def payments_menu_kb(
    ...
    # Ручные переводы на карту (без эквайринга) — отображаем только информацию и настройки
    builder.row(
        InlineKeyboardButton(
            text="💸 Ручные переводы на карту",
            callback_data="admin_manual_payments_info"
        )
    )
...
def broadcast_menu_kb() -> InlineKeyboardMarkup:
    ...
    # Основные функции
    builder.row(
        InlineKeyboardButton(text="✉️ Сообщение", callback_data="admin_broadcast_edit_message")
    )
    builder.row(
        InlineKeyboardButton(text="🎯 Фильтр", callback_data="admin_broadcast_filter")
    )
    builder.row(
    ...
    # Автоуведомления
    builder.row(
        InlineKeyboardButton(text="⏰ Автоуведомления", callback_data="admin_broadcast_notifications")
    )
...
def broadcast_filter_kb(current_filter: str) -> InlineKeyboardMarkup:
    ...
    for filter_key, filter_name in BROADCAST_FILTERS.items():
        ...
        builder.row(
            InlineKeyboardButton(
                text=f"{radio} {filter_name}",
                callback_data=f"admin_broadcast_filter_set:{filter_key}"
            )
        )
...
def users_list_kb(
    ...
    Args:
        ...
        mode: 'view' — переход в карточку пользователя; 'grant_sub' — выдача подписки
    """
    builder = InlineKeyboardBuilder()
    
    is_grant_sub = mode == 'grant_sub'
    
    # Фильтры в одну строку (в режиме выдачи подписки тоже показываем)
    filter_buttons = []
    ...
    for user in users:
        ...
        callback = f"admin_grant_sub_user:{telegram_id}" if is_grant_sub else f"admin_user_view:{telegram_id}"
        builder.row(
            InlineKeyboardButton(
                text=text,
...
def add_key_tariffs_kb(tariffs: List[Dict[str, Any]]) -> InlineKeyboardMarkup:
    """
    Клавиатура выбора тарифа при добавлении ключа / выдаче подписки.
    
    Args:
        tariffs: Список тарифов (активных)
    """
    builder = InlineKeyboardBuilder()
    
    for tariff in tariffs:
        if not tariff.get('is_active'):
            continue
        builder.row(
            InlineKeyboardButton(
                text=f"📋 {tariff['name']} ({tariff['duration_days']} дн.)",
                callback_data=f"admin_add_key_tariff:{tariff['id']}"
            )
        )
...
def manual_payment_review_kb(user_telegram_id: int, tariff_id: int) -> InlineKeyboardMarkup:
    """
    Клавиатура под чеком ручного перевода на карту в админ-чате.

    Args:
        user_telegram_id: Telegram ID плательщика
        tariff_id: ID тарифа (для автоматической выдачи ключа при подтверждении)
    """
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(
            text="✅ Подтвердить оплату",
            callback_data=f"admin_manual_payment_confirm:{user_telegram_id}:{tariff_id}"
        ),
        InlineKeyboardButton(
            text="❌ Отклонить",
            callback_data=f"admin_manual_payment_reject:{user_telegram_id}"
        ),
    )
    return builder.as_markup()
