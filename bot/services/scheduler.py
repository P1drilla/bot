...
from database.connection import DB_PATH as BOT_DB_PATH
...
async def collect_daily_stats() -> str:
    ...
    users = await get_users_stats()
    new_users = await get_new_users_count_today()
    
    # Статистика ключей
    keys = await get_keys_stats()
    
    # Статистика платежей
    payments = await get_daily_payments_stats()
...
async def create_backup_archive() -> Optional[bytes]:
    ...
    try:
        ...
        with zipfile.ZipFile(archive_buffer, 'w', zipfile.ZIP_DEFLATED) as zf:
            ...
            bot_db_path = BOT_DB_PATH.resolve()
            if bot_db_path.exists():
                zf.write(bot_db_path, 'vpn_bot.db')
                logger.info(f"Добавлен в архив: vpn_bot.db ({bot_db_path.stat().st_size} байт)")
            else:
...
async def check_and_send_expiry_notifications(bot: Bot) -> None:
    ...
    try:
        days = int(await get_setting('notification_days', '3'))
        notification_text = await get_setting(
            'notification_text',
            '⚠️ *Ваш ключ для оптимизации трафика скоро истекает!*\n\n'
            'Через {days} дней закончится срок действия вашего ключа.\n\n'
            'Продлите подписку, чтобы сохранить доступ к сервису без перерыва!'
        )
        
        expiring_keys = await get_expiring_keys(days)
        sent_count = 0
        ...
        for key_info in expiring_keys:
            ...
            if await is_notification_sent_today(vpn_key_id):
                continue
            ...
            try:
                ...
                await log_notification_sent(vpn_key_id)
                sent_count += 1
            except Exception as e:
        ...
        logger.exception("Ошибка в check_and_send_expiry_notifications")
...
async def run_daily_tasks(bot: Bot) -> None:
    ...
    while True:
        try:
            ...
            logger.exception("Ошибка в планировщике ежедневных задач")
