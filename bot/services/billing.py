...
async def apply_referral_bonus_if_needed(order: Dict[str, Any]) -> None:
    """
    Начисляет реферальный бонус пригласившему пользователю, если выполняются условия.
    
    Условия:
    - Реферальная программа включена (referral_enabled = '1')
    - У пользователя указан invited_by_user_id
    - За этого пользователя ещё не был выдан бонус (referral_bonus_given = 0)
    - У пригласившего есть хотя бы один VPN-ключ
    """
    try:
        from database.requests import (
            get_user_by_id,
        ...
        enabled = await get_setting("referral_enabled", "1")
        if enabled != "1":
            return
        ...
        # Бонус уже выдавался за этого пользователя
        ...
        # Получаем все ключи пригласившего
        referrer_keys = await get_user_vpn_keys(invited_by_user_id)
        if not referrer_keys:
            logger.info(
                "Реферальный бонус не применён: у пригласившего нет ключей "
                f"(invited_user_id={user_id}, referrer_user_id={invited_by_user_id})"
            )
            return
        ...
        for key in referrer_keys:
            ...
            if not key_id:
                continue
            if await extend_vpn_key(key_id, bonus_days):
                extended_count += 1

        if extended_count > 0:
            await mark_referral_bonus_given(user_id)
            logger.info(
                "Реферальный бонус применён: user_id=%s, referrer_user_id=%s, "
                "bonus_days=%s, extended_keys=%s",
                user_id,
                invited_by_user_id,
                bonus_days,
                extended_count,
            ...
            logger.warning(
                "Реферальный бонус не смог продлить ни один ключ "
                "(user_id=%s, referrer_user_id=%s)",
                user_id,
                invited_by_user_id,
            )
    except Exception as e:
        # Ошибка бонуса не должна ломать обработку платежей
        logger.error(f"Ошибка применения реферального бонуса: {e}")
...
async def process_payment_order(order_id: str) -> Tuple[bool, str, Optional[Dict[str, Any]]]:
    ...
    # 3. Закрываем ордер
    if not await complete_order(order_id):
    ...
    # 3.1. Пробуем начислить реферальный бонус (ошибки не ломают платеж)
    await apply_referral_bonus_if_needed(order)

    # 4. Обработка ключа (Продление или Создание)
    if order['vpn_key_id']:
        # Продление
        days = order.get('period_days') or order.get('duration_days')
        if days and await extend_vpn_key(order['vpn_key_id'], days):
...
async def process_crypto_payment(start_param: str, user_id: Optional[int] = None) -> Tuple[bool, str, Optional[Dict[str, Any]]]:
    ...
    secret_key = await get_setting('crypto_secret_key')
    if not secret_key:
    ...
    if order and parsed.get('tariff') and parsed['tariff'] != '_':
        try:
            ...
            real_tariff = await get_tariff_by_external_id(tariff_ext_id)
            ...
            if real_tariff and (real_tariff['id'] != order['tariff_id'] or order.get('payment_type') != 'crypto'):
                ...
                if await update_order_tariff(order_id, real_tariff['id'], payment_type='crypto'):
    ...
    if not order:
        ...
        if parsed.get('tariff') and parsed['tariff'] != '_':
            try:
                ...
                tariff = await get_tariff_by_external_id(tariff_external_id)
                if tariff:
                    tariff_id = tariff['id']
        ...
        success = await create_paid_order_external(
            order_id=order_id,
            user_id=user_id,
            tariff_id=tariff_id,
            payment_type='crypto',
