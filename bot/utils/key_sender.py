...
async def send_key_with_qr(
    ...
    try:
        ...
        if not config:
            ...
            text = (
                f"📋 *Ваш ключ для оптимизации трафика*\n\n"
                f"```\n{uuid}\n```\n\n"
        ...
        title = "✅ *Ваш новый ключ для оптимизации трафика!*" if is_new else "📋 *Ваш ключ для оптимизации трафика*"
        caption = (
            f"{title}\n\n"
            f"```\n{link}\n```\n"
            ...
            "2. Импортируйте в V2RayNG / Hiddify / Happ.\n"
